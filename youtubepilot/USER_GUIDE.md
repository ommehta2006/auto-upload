...
Open **Channel -> Connect YouTube Studio**. Sign in only inside the remote Google/YouTube page. Complete verification and wait for the Studio dashboard. Return and save the connection. The server keeps one persistent Chrome profile for that account and reuses it for future uploads. The application stores encrypted browser-state backups, not the Google password.

Avoid reconnecting from different servers, regions, browser locales or browser timezones unless Google asks for verification. If Google shows OTP, CAPTCHA or "verify it's you", complete it manually in the remote browser and save the connection again.
