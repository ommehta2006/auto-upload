...
async function humanClick(locator, timeout = 2500) {
  await humanPause();
  await locator.first().click({ timeout, delay: randomInt(35, 120) });
}

async function humanFill(locator, value, timeout = 3000) {
  const target = locator.first();
  const text = String(value);
  const typeOrFill = async () => {
    if (text.length <= 350 && typeof target.pressSequentially === 'function') {
      await target.pressSequentially(text, { delay: randomInt(8, 24), timeout });
    } else {
      await target.fill(text, { timeout });
    }
  };
  await humanPause(180, 520);
  await target.click({ timeout, delay: randomInt(30, 110) }).catch(() => {});
  if (await target.getAttribute('contenteditable').catch(() => null)) {
    await target.press(process.platform === 'darwin' ? 'Meta+A' : 'Control+A');
    await typeOrFill().catch(() => target.fill(text, { timeout }));
  } else {
    await target.fill('', { timeout }).catch(() => {});
    await typeOrFill().catch(() => target.fill(text, { timeout }));
  }
}
...
async function clickCandidates(candidates, timeout = 2500) {
  for (const candidate of candidates) {
    ...
    if (await visible(locator, Math.min(timeout, 1500))) {
      try { await humanClick(locator, timeout); return true; } catch {}
    }
  }
  return false;
}
...
async function fillCandidates(candidates, value, timeout = 3000) {
  for (const candidate of candidates) {
    ...
    try {
      const target = locator.first();
      await humanFill(target, value, timeout);
      return true;
    } catch {}
  }
  return false;
...
async function openDropdownNearLabel(page, labelPattern) {
  ...
  if (await visible(trigger, 500)) return humanClick(trigger,2500).then(() => true).catch(() => false);
  return humanClick(label,2500).then(() => true).catch(() => false);
}
...
async function selectDropdown(page, labelPattern, optionPattern) {
  ...
  return humanClick(option,2500).then(() => true).catch(() => false);
}
...
async function nextStep(page) {
  ...
  if (await visible(button, 1000)) {
    ...
    for (let i=0;i<30;i+=1) {
      ...
      if (disabled === null && ariaDisabled !== 'true') { await humanClick(button,4000); return true; }
...
export async function uploadToYouTube({ post, storageState, profileDir, videoPath, thumbnailPath, captionPath, screenshotPath, log = () => {} }) {
  const warnings = [];
  const probe = validateContentType(await probeVideo(videoPath), post.content_type);
  log('info','Video file validated.',probe);
  let context; let page; let popupTimer; let releaseProfile;
  try {
    releaseProfile = acquireUserBrowserProfile(post.user_id);
    ...
    await restoreBrowserCookies(context,storageState);
