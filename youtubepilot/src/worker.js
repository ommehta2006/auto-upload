...
import { ensureUserBrowserProfile } from './services/chrome-runtime.js';
...
async function processUpload(post) {
  ...
  try {
    ...
    const profileDir = await ensureUserBrowserProfile(post.user_id);
    ...
    const result = await uploadToYouTube({ post,storageState:state,profileDir,videoPath:mediaPath,thumbnailPath,captionPath,screenshotPath,log });
