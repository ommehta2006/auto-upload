...
- Persistent per-user Google Chrome profiles plus encrypted Playwright storage-state backups; Google passwords are never stored.
...
Customer browser
  ...
  -> private persistent media/profile volume
  -> background worker
  -> Google Chrome Stable under Xvfb
  -> YouTube Studio
```
